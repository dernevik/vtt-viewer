...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:tt="http://www.vector-informatik.de/ITE/TestTable/1.0" exclude-result-prefixes="tt">
    <xsl:output method="html" encoding="UTF-8"/>
    <!--
  VTT → HTML viewer

  Purpose:
    Render Vector TestTable (.vtt) fixtures as human-readable HTML.

  Inputs:
    • $fixture — optional fixture title to filter on (exact, trimmed)
    • $showStepNumbers — reserved for future numbering (default false)
    • $showActiveFlag — reserved (default false)
    • $softenPaths — pretty print DBSignal/SysVar/PDU paths (default true)

  Namespace strategy:
    Use the 'tt:' namespace plus local-name() fallbacks to be resilient to
    schema variations across exports.

  XSLT level:
    XSLT 1.0 for .NET XslCompiledTransform. Avoid EXSLT node-set hacks.

  Conventions:
    • Code-like text goes inside <code>
    • Normalize whitespace with normalize-space() before compare/print
    • On missing data, prefer a graceful omission over a literal question mark
    • Prefer namespace-agnostic matches where helpful

  Adding a new step:
    1) Add template: match="tt:name | *[local-name()='name']" mode="step"
    2) Extract title/value safely; for case-insensitive tests use translate()
...
-->
  ...
  - tc-by-name:    by <title|name> text (trimmed)
  - fn-by-name:    any element that has a child <name>, trimmed
  - var-by-name:   variable_definition by <name>
  Used by inline expansion and cross-references.
-->
    <xsl:key name="tc-by-id" match="tt:tc|tt:tc_definition|*[@tcid]" use="normalize-space(tt:tcid|@tcid)"/>
    <xsl:key name="tc-by-name" match="tt:tc|tt:tc_definition|*[*[local-name()='title' or local-name()='name']]" use="normalize-space(tt:title|tt:name|*[local-name()='title']|*[local-name()='name'])"/>
    <xsl:key name="fn-by-name" match="*[*[local-name()='name']]" use="normalize-space(*[local-name()='name'])"/>
    <xsl:key name="var-by-name" match="*[(local-name()='variable_definition')]" use="normalize-space(*[local-name()='name'])"/>
    <!--
  bestValueLabel(ctx)
  Returns a concise label for a <value>-shaped node:
    • const                → text of <const>
    • valuetable_entry     → text
    • dbobject             → prettyPath(dbobject string)
    • variable             → "$VarName"
    • otherwise            → normalized string(ctx)
-->
    <xsl:template name="bestValueLabel">
        ...
        <xsl:variable name="tc" select="(ancestor::tt:tc | ancestor::tt:tc_definition |
        ...
        <xsl:variable name="localConst" select="normalize-space($tc//*[local-name()='variable_definition']
					  [normalize-space(*[local-name()='name'])=$varName]
					  [last()]/*[local-name()='source']/*[local-name()='value']/*[local-name()='const'])"/>
        <xsl:variable name="globalConst" select="normalize-space((key('var-by-name',$varName))[1]
    ...
    <!--
  Path softening
  prettyPath(text) collapses long BEGIN_OF_OBJECT / END_OF_OBJECT chains
  into one readable label.

  Rules:
    • DBSignal → "DBSignal <SignalName>" or "<PDU>.<Signal>" when available
    • SysVar   → "SysVar <ns>::<name>"
    • PDU      → "PDU <Name>" via pduLabel()
    • Fallback → original string when parsing is uncertain

  Implementation:
    BEGIN_/END_ markers removed; last-token / join-namespace isolate leaf vs ns.
    Always favor safe fallback over a misleading reconstruction.
-->
    <xsl:template name="prettyPath">
        ...
        <xsl:variable name="preEnd" select="substring-before(concat($noBegin,'|END_OF_OBJECT'),'|END_OF_OBJECT')"/>
        ...
        <xsl:choose>
            ...
            <xsl:when test="starts-with($raw,'SysVar')">
                ...
                <xsl:variable name="core1">
                    <xsl:choose>
                        <xsl:when test="starts-with($h0,'SysVar')">
                            <xsl:value-of select="$t0"/>
                        </xsl:when>
                        <xsl:otherwise>
                            <xsl:value-of select="$core0"/>
                        </xsl:otherwise>
                    </xsl:choose>
                </xsl:variable>
                ...
                <xsl:variable name="pdu">
                    <xsl:choose>
                        <xsl:when test="contains($t1,'|')">
                            <xsl:call-template name="last-token">
                                <xsl:with-param name="s" select="$t1"/>
                            </xsl:call-template>
                        </xsl:when>
                        <xsl:otherwise>
    ...
    <!--
  pduLabel(path)
  Extract the real PDU name from a DBFrameOrPDU path while ignoring
  trailing boolean/flag fields. Used by OCCURRENCE_COUNT.
-->
    <xsl:template name="pduLabel">
        <xsl:param name="path"/>
        <!-- Prefer the FrameData slice: ... FrameData_BEGIN_OF_OBJECT | ... | END_OF_OBJECT_FrameData -->
        <xsl:variable name="frameSlice" select="substring-before(
        ...
        <xsl:choose>
            ...
            <xsl:otherwise>
                <xsl:variable name="base" select="substring-before(concat($path,'|END_OF_OBJECT_'),'|END_OF_OBJECT_')"/>
    ...
    <xsl:template name="strip-begin-markers">
        ...
        <xsl:choose>
            <xsl:when test="contains($s,'_BEGIN_OF_OBJECT|')">
                <xsl:call-template name="strip-begin-markers">
                    <xsl:with-param name="s" select="concat(substring-before($s,'_BEGIN_OF_OBJECT|'),
    ...
    <xsl:template name="format-DBSignal">
        ...
        <xsl:variable name="core1">
            <xsl:choose>
                <xsl:when test="starts-with($h0,'DBSignal')">
                    <xsl:value-of select="$t0"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="$core0"/>
        ...
        <xsl:variable name="core2">
            <xsl:choose>
                <xsl:when test="starts-with($h1,'NodeOrFrameData')">
                    <xsl:value-of select="$t1"/>
                </xsl:when>
                <xsl:otherwise>
                    <xsl:value-of select="$core1"/>
                </xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
    ...
    <xsl:template name="diagServiceLabel">
        ...
        <xsl:variable name="base" select="substring-before(concat($path,'|END_OF_OBJECT'), '|END_OF_OBJECT')"/>
    ...
    <xsl:template name="lastAlphaToken">
        ...
        <xsl:choose>
            <xsl:when test="contains($s,'|')">
                ...
                <xsl:variable name="hasAlpha" select="string-length($tok) &gt; string-length(translate($tok,
                 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz',''))"/>
                <xsl:variable name="tokU" select="translate($tok,'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ')"/>
                <xsl:choose>
    ...
    <!--
  join-params-html(ctx)
  Render function/step parameters as a single comma-separated string:
    - String types → quoted
    - Numeric consts → raw digits
    - valuetable_entry → text
    - dbobject → prettyPath()
    - variable → $Name
  Robust to both <param><value>… and flattened shapes.
-->
    <xsl:template name="join-params-html">
        <xsl:param name="ctx"/>
        <xsl:for-each select="$ctx//*[local-name()='param']">
            <xsl:if test="position() &gt; 1">
                <xsl:text>,</xsl:text>
            </xsl:if>
            <xsl:call-template name="paramLabel">
                <xsl:with-param name="p" select="."/>
            </xsl:call-template>
        </xsl:for-each>
    </xsl:template>
    <!-- Render a single NETFUNC param with no leading/trailing spaces -->
    <xsl:template name="paramLabel">
        <xsl:param name="p"/>
        <!-- type (for quoting decision) -->
        <xsl:variable name="t" select="normalize-space(($p/*[local-name()='type'])[1])"/>
        <!-- value node -->
        <xsl:variable name="v" select="($p/*[local-name()='value'])[1]"/>
        <!-- try valuetable_entry / const / dbobject / path first -->
        <xsl:variable name="lit" select="normalize-space(($v//*[local-name()='valuetable_entry'
                                  or local-name()='const'
                                  or local-name()='dbobject'
                                  or local-name()='path'])[1])"/>
        <!-- fallback: raw string content -->
        <xsl:variable name="raw" select="normalize-space(string($v))"/>
        ...
        <!-- Quote only String params -->
        <xsl:choose>
            <xsl:when test="translate($t,'abcdefghijklmnopqrstuvwxyz','ABCDEFGHIJKLMNOPQRSTUVWXYZ')='STRING'">
                <xsl:text>"</xsl:text>
    ...
    <!--
  Root:
  If $fixture is set, render any fixture with an equal (trimmed) title.
  Otherwise render all top-level fixtures. Also list global test case
  definitions when not filtered.
-->
    <xsl:template match="/">
        <html>
            <head>
    ...
    <xsl:template match="tt:tf">
        <div class="fixture" id="{normalize-space(tt:title)}">
            <h2>Fixture: <xsl:value-of select="normalize-space(tt:title)"/>
            </h2>
    ...
    <xsl:template match="tt:tttestcase">
        ...
        <li>
            ...
            <xsl:choose>
                <xsl:when test="$byId">
                    <ul>
                        <xsl:apply-templates select="$byId" mode="inline"/>
                    </ul>
                </xsl:when>
                <xsl:when test="$byName">
                    <span class="muted">— matched by name</span>
                    <ul>
                        <xsl:apply-templates select="$byName" mode="inline"/>
                    </ul>
                </xsl:when>
                <xsl:otherwise>
                    <span class="muted">— definition not found</span>
                </xsl:otherwise>
            </xsl:choose>
        </li>
    </xsl:template>
    ...
    <xsl:template match="tt:tc | tt:tc_definition" mode="inline">
        <xsl:if test="tt:preparation">
            <li>
                ...
                <ul>
                    <xsl:apply-templates select="tt:preparation/tt:*" mode="step"/>
                </ul>
            </li>
        </xsl:if>
        ...
        <xsl:if test="tt:completion">
            <li>
                ...
                <ul>
                    <xsl:apply-templates select="tt:completion/tt:*" mode="step"/>
                </ul>
            </li>
        </xsl:if>
    </xsl:template>
    ...
    <xsl:template match="tt:preparation | tt:completion">
        <h3>
            <xsl:choose>
                <xsl:when test="self::tt:preparation">Preparation</xsl:when>
                <xsl:otherwise>Completion</xsl:otherwise>
            </xsl:choose>
        </h3>
        <ul>
            <xsl:apply-templates select="tt:*" mode="step"/>
        </ul>
    </xsl:template>
    <!-- ========== Step renderers ========== -->
                ...
                <xsl:for-each select="*[local-name()='in']/*[local-name()='assignment']">
                    <li>
                        <code>
                            ...
                            <xsl:call-template name="prettyPath">
                                <xsl:with-param name="text" select="normalize-space(*[local-name()='sink']/*[local-name()='dbobject'])"/>
                            </xsl:call-template>
                            <xsl:text>=</xsl:text>
                            ...
                            <xsl:choose>
                                ...
                                <xsl:when test="normalize-space(*[local-name()='source']/*[local-name()='dbobject'])!=''">
                                    <xsl:call-template name="prettyPath">
                                        <xsl:with-param name="text" select="normalize-space(*[local-name()='source']/*[local-name()='dbobject'])"/>
                                    </xsl:call-template>
                                </xsl:when>
                                <!-- fallback -->
                                <xsl:otherwise>
    ...
    <xsl:template match="tt:foreach" mode="step">
        <li>
            <code>FOREACH<xsl:value-of select="normalize-space(tt:loopvar)"/>
            </code>
            <ul>
                <xsl:apply-templates select="tt:*[
    ...
    <xsl:template match="tt:statecheck | *[local-name()='statecheck']" mode="step">
        <li>
            ...
            <ul>
                <!-- Optional title/name -->
                <xsl:variable name="ttl" select="normalize-space( (tt:title | *[local-name()='title'] | tt:name | *[local-name()='name'])[1] )"/>
                <xsl:if test="$ttl!=''">
                    <li>
                        <em>TITLE: </em>
                        <span class="muted">
    ...
    <xsl:template match="tt:compare | *[local-name()='compare']" mode="step">
        ...
        <xsl:variable name="op">
            <xsl:choose>
                <xsl:when test=".//*[local-name()='eq']">==</xsl:when>
                <xsl:when test=".//*[local-name()='ne']">!=</xsl:when>
                <xsl:when test=".//*[local-name()='ge']">&gt;=</xsl:when>
                <xsl:when test=".//*[local-name()='gt']">&gt;</xsl:when>
                <xsl:when test=".//*[local-name()='le']">&lt;=</xsl:when>
                <xsl:when test=".//*[local-name()='lt']">&lt;</xsl:when>
                <xsl:otherwise>==</xsl:otherwise>
            </xsl:choose>
        </xsl:variable>
        <!-- LHS -->
        <xsl:variable name="lhsNode" select="(.//*[local-name()='lhs']/*[local-name()='dbobject']
           | .//*[local-name()='left']/*[local-name()='dbobject']
           | .//*[local-name()='dbobject'])[1]"/>
        <xsl:variable name="lhsLabel">
            <xsl:choose>
                <xsl:when test="$lhsNode">
                    <xsl:call-template name="prettyPath">
                        <xsl:with-param name="text" select="normalize-space(string($lhsNode))"/>
                    </xsl:call-template>
                </xsl:when>
                <xsl:otherwise>
        ...
        <!-- RHS -->
        <xsl:variable name="rhsOp" select="(./*[local-name()='eq' or local-name()='ne' or local-name()='gt' or local-name()='ge' or local-name()='lt' or local-name()='le'])[1]"/>
        <xsl:variable name="rhsLabel">
            <xsl:choose>
                <xsl:when test="$rhsOp//*[local-name()='value']">
                    <xsl:call-template name="bestValueLabel">
                        <xsl:with-param name="ctx" select="$rhsOp//*[local-name()='value'][1]"/>
                    </xsl:call-template>
                </xsl:when>
                <xsl:when test="$rhsOp//*[local-name()='dbobject']">
                    <xsl:call-template name="prettyPath">
                        <xsl:with-param name="text" select="normalize-space(string($rhsOp//*[local-name()='dbobject'][1]))"/>
                    </xsl:call-template>
                </xsl:when>
                <xsl:when test="normalize-space(string($rhsOp))!=''">
                    <xsl:value-of select="normalize-space(string($rhsOp))"/>
                </xsl:when>
                <xsl:otherwise/>
            </xsl:choose>
        </xsl:variable>
        <li>
            <code>
                <xsl:value-of select="$lhsLabel"/>
                <xsl:value-of select="$op"/>
                <xsl:if test="normalize-space($rhsLabel)!=''">
                    <xsl:value-of select="$rhsLabel"/>
                </xsl:if>
            </code>
        </li>
    </xsl:template>
    ...
    <!-- VARIABLE_DEFINITION (namespace-agnostic) -->
<xsl:template match="tt:variable_definition | *[local-name()='variable_definition']" mode="step">
  <li>
    <code>
      VARIABLE_DEFINITION
      <xsl:text> </xsl:text>
      ...
      <!-- :type -->
      <xsl:text>:</xsl:text>
      <xsl:value-of select="normalize-space((tt:type | *[local-name()='type'])[1])"/>

      <!-- ← value (if any) -->
      <xsl:variable name="valNode"
        select="(tt:value | *[local-name()='value'] | .//*[local-name()='value'])[1]"/>
      <xsl:variable name="hasVal"
        select="boolean($valNode | .//*[local-name()='valuetable_entry']
                         | .//*[local-name()='const']
                         | .//*[local-name()='dbobject']
                         | .//*[local-name()='path'])"/>
      ...
      <xsl:if test="$hasVal">
        ...
        <xsl:text> </xsl:text>
        <xsl:call-template name="bestValueLabel">
          <xsl:with-param name="ctx" select="$valNode"/>
        </xsl:call-template>
      </xsl:if>
    </code>
  </li>
    ...
    <xsl:template match="tt:assignment" mode="step">
        <li>
            <code>
                ...
                <xsl:text>=</xsl:text>
                <xsl:choose>
                    <xsl:when test="tt:source/tt:valuetable_entry">
                        <xsl:value-of select="normalize-space(tt:source/tt:valuetable_entry)"/>
                    </xsl:when>
                    <xsl:when test="tt:source/tt:value/tt:const">
    ...
    <!-- TITLE: emit only when non-empty, after normalizing sources -->
    <xsl:template match="tt:title" mode="step">
        ...
        <xsl:if test="$t!=''">
            <li>
                <em>TITLE</em>: <xsl:value-of select="$t"/>
            </li>
        </xsl:if>
    </xsl:template>
    <!-- OCCURRENCE_COUNT: optional title; limits; objects list (PDU-aware) -->
    ...
    <xsl:template match="tt:check_deactivation | *[local-name()='check_deactivation']" mode="step">
        <li>
            ...
            <xsl:variable name="cid" select="normalize-space( (tt:checkid | @checkid | @id | *[local-name()='checkid'] | @*[local-name()='checkid'] | @*[local-name()='id'])[1] )"/>
            <xsl:variable name="ttl" select="normalize-space( (tt:title | *[local-name()='title'] | tt:name | *[local-name()='name'])[1] )"/>
            <xsl:choose>
                <xsl:when test="$cid!=''">
                    <span class="muted">— id=<code>
                            <xsl:value-of select="$cid"/>
                        </code>
                    </span>
                </xsl:when>
                <xsl:when test="$ttl!=''">
                    <span class="muted">—<xsl:value-of select="$ttl"/>
                    </span>
                </xsl:when>
                <xsl:otherwise>
            ...
            <xsl:variable name="objs" select=".//*[local-name()='objects']/* | .//*[local-name()='pdu' or local-name()='dbobject' or local-name()='eventsource']"/>
            <xsl:if test="$objs">
                <ul>
                    <xsl:for-each select="$objs">
                        <li>
                            <code>
                                ...
                                <xsl:call-template name="prettyPath">
                                    <xsl:with-param name="text" select="normalize-space( (./tt:dbobject | ./*[local-name()='dbobject'] | .)[1] )"/>
                                </xsl:call-template>
                            </code>
                        </li>
                    </xsl:for-each>
    ...
    <xsl:template match="tt:novaluechange | *[local-name()='novaluechange']" mode="step">
        <li>
            ...
            <xsl:variable name="tTitle" select="normalize-space( (tt:title | *[local-name()='title'])[1] )"/>
            <xsl:if test="$tTitle!=''">
                <span class="muted">—<xsl:value-of select="$tTitle"/>
                </span>
            </xsl:if>
            <!-- gather any explicit objects or dbobject paths under this node -->
            <xsl:variable name="candidates" select=".//*[local-name()='objects']/* | .//*[local-name()='dbobject']"/>
            <ul>
                <xsl:choose>
    ...
    <xsl:template match="tt:diagservice | *[local-name()='diagservice']" mode="step">
        <li>
            ...
            <xsl:choose>
                ...
                <xsl:otherwise>
                    <span class="muted">no service specified</span>
                </xsl:otherwise>
            </xsl:choose>
            <ul>
                <xsl:if test="*[local-name()='diagrequest']">
                    <li class="muted">request present</li>
                </xsl:if>
                <xsl:if test="*[local-name()='diagresponse']">
                    <li class="muted">response present</li>
                </xsl:if>
            </ul>
        </li>
    </xsl:template>
    ...
    <!-- CAPLFUNCTION: name(<joined args>); mirrors NETFUNC formatting -->
    <!-- after -->
    <xsl:template match="tt:caplfunction | *[local-name()='caplfunction']" mode="step">
        <li>
            <code>
    ...
    <!-- Fallback step: uppercase tag; show muted subtitle from title/text/message -->
    <xsl:template match="tt:*" mode="step">
        <li>
            ...
            <xsl:if test="$msg!=''">
                <span class="muted">—<xsl:value-of select="$msg"/>
                </span>
            </xsl:if>
        </li>
    </xsl:template>
    ...
    <!--
  Maintenance tips
  • normalize-space() before comparisons and output
  • translate() for case-insensitive tests
  • prefer *[local-name()='node'] when schemas vary
  • print values via bestValueLabel() rather than raw string()
  • for timing (WAIT/TIMEOUT) print a space before the unit
  • when in doubt, fall back to raw content rather than guessing
-->
</xsl:stylesheet>
